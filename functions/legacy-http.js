...
const RESULTS_EMAIL_RECIPIENT = "congiirepair@gmail.com";
const RESULTS_EMAIL_FROM = "Prodigy Event Control <onboarding@resend.dev>";
...
const parseVoiceDeductions = onRequest({
  region: "us-central1",
  cors: true,
  maxInstances: 10,
  timeoutSeconds: 20,
...
function isValidBase64Pdf(value) {
  return typeof value === "string" && value.length > 0 && value.length <= MAX_PDF_BYTES * 1.4 && /^[A-Za-z0-9+/]+=*$/.test(value);
}

const emailEventResultsSummary = onRequest({
  region: "us-central1",
  cors: true,
  maxInstances: 10,
  timeoutSeconds: 30,
  ...
  const eventName = String(request.body?.eventName || "Prodigy Event").trim().slice(0, 200);
  const pdfBase64 = request.body?.pdfBase64;
  if (!isValidBase64Pdf(pdfBase64)) {
    sendJson(response, 400, { error: "Missing or invalid pdfBase64" });
    return;
  }

  const fileName = `${eventName.replace(/[^A-Za-z0-9 _-]/g, "").trim() || "prodigy-event"}-results.pdf`;
  ...
  try {
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [RESULTS_EMAIL_RECIPIENT],
        subject: `Results Summary - ${eventName}`,
        text: `Attached: registered drivers, qualifying scores (best to worst), and podium results for ${eventName}.`,
        attachments: [
          {
            filename: fileName,
            content: pdfBase64,
    ...
    if (!emailResponse.ok) {
      ...
      console.error("Resend send failed", emailResponse.status, detail.slice(0, 500));
      sendJson(response, 502, { error: "Email send failed" });
      return;
    }
    ...
    sendJson(response, 500, { error: "Email unavailable" });
  }
});
