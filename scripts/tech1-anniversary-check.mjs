...
function runRepoReadinessChecks() {
  ...
  const routeTest = fs.existsSync("tests/e2e/tech1-anniversary.spec.js");
  addCheck(checks, "public /tech1 route test exists", routeTest);
  ...
  addCheck(checks, "public /tech1 test hides staff raffle form", e2eSource.includes("#tech1RaffleForm") && e2eSource.includes("toHaveCount(0)"));
  ...
  addCheck(checks, "public registration guards missing event shell", appSource.includes("Event staff are opening Tech 1 registration"));
  addCheck(checks, "staff claim requirement is documented", docsExists && fs.readFileSync("docs/TECH1_DRIFT_ANNIVERSARY_MODE.md", "utf8").includes("claims:set"));
  return checks;
}
